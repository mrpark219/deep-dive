...
- 웹 애플리케이션 서버(Web Application Server: **WAS**)의 이름에 Web Server 대신 Application이 들어간 이유는, 단순한 웹 서버의 역할을 넘어 추가로 애플리케이션, 즉 **프로그램 코드도 수행**할 수 있는 서버라는 뜻이다.
- 즉, 웹을 기반으로 작동하면서 개발자가 작성한 복잡한 비즈니스 로직(프로그램 코드)도 함께 실행할 수 있는 서버를 의미한다.
- HTTP와 웹이 처음 등장했을 때, 수많은 회사에서 직접 HTTP 서버와 비슷한 기능을 독자적으로 개발하기 시작했고 이들 장비 간의 **호환성**은 전혀 없었다.
- 따라서 A 회사의 서버를 사용하다가 B 회사의 서버로 변경하려면, 서버마다 통신 인터페이스가 모두 달랐기 때문에 기존 코드를 대대적으로 수정해야만 하는 큰 불편함이 있었다.

### 7.1. 서블릿(Servlet)과 웹 애플리케이션 서버

- 이러한 심각한 호환성 문제를 해결하기 위해 1990년대 자바 진영에서 **서블릿(Servlet)** 이라는 강력한 기술 표준이 등장하게 되었다.
- 서블릿은 `Servlet`, `HttpServlet`, `ServletRequest`, `ServletResponse` 등 자바 웹 프로그래밍을 위한 다양한 인터페이스와 **표준 규격**을 제공한다.
- 이후 자바 기반의 HTTP 서버를 만드는 회사들은 모두 이 서블릿 표준 규격을 기반으로 서버 엔진을 구현하여 제공하기 시작했다.
- 서블릿 스펙을 완벽하게 지원하는 대표적인 자바 웹 애플리케이션 서버(WAS)로는 **Apache Tomcat, Jetty, GlassFish, Undertow** 등이 있다.

### 7.2. 표준화의 장점

- HTTP 서버 제조사들이 공통된 서블릿 표준을 기반으로 기능을 제공한 덕분에, 개발자는 단순히 **`jakarta.servlet.Servlet` 인터페이스** 규격에 맞춰 로직을 구현하기만 하면 된다.
- 이렇게 작성된 Servlet 구현체는 서블릿 규격을 준수하는 어떤 애플리케이션 서버(Tomcat 등)에 올려도 문제없이 정상적으로 실행된다.
- 향후 트래픽 증가나 성능 개선 등의 이유로 기존 서버를 다른 종류의 WAS로 변경할 때도, **코드 수정 없이 기존에 구현한 서블릿을 그대로 사용**할 수 있다.
- 이것이 바로 **표준화의 가장 큰 장점**이며, 개발자는 코드 변경 부담 없이 서버 환경을 자유롭게 변경할 수 있고, 서버 제조사는 호환성 걱정 없이 더 나은 성능과 부가 기능을 개발하는 데 집중할 수 있게 되었다.
