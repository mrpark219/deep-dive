...
- **버전 관리의 어려움**: 클래스 구조가 변경되면 이전에 직렬화된 객체와 호환성 문제가 발생하며, `serialVersionUID` 관리가 복잡하다.
- **플랫폼 종속성**: 자바 플랫폼에 깊게 종속되어 있어, 다른 언어나 시스템과의 상호 운용성이 크게 떨어진다.
- **성능 이슈**: 직렬화 및 역직렬화 과정이 상대적으로 느리고 시스템 리소스를 많이 사용한다.
- **유연성 부족**: 직렬화된 데이터 형식을 개발자가 원하는 대로 커스터마이즈하기 어렵다.
- **크기 효율성**: 직렬화된 데이터 자체의 크기가 상대적으로 커서 비효율적이다.
...
- 자바 직렬화의 플랫폼 종속성 문제를 해결하기 위해 2000년대 초반에 **XML**이라는 기술이 큰 인기를 끌었다.
- 매우 유연하고 강력한 기술이었지만, 태그를 포함한 문서의 크기가 커서 네트워크 전송 비용이 증가하는 등 **복잡성**과 **무거움**이라는 뚜렷한 단점이 있었다.
...
- XML의 대안으로 등장한 **JSON**은 구조가 **가볍고 간결하며**, 자바스크립트와의 자연스러운 호환성 덕분에 빠르게 확산되었다.
- 2000년대 후반 웹 API와 RESTful 서비스가 대중화되면서, JSON은 **표준 데이터 교환 포맷**으로 확고히 자리 잡았다.
- 데이터 구조의 복잡성과 엄격한 스키마 정의가 필요했던 초기 엔터프라이즈 환경에서는 XML이 중요했지만, 웹과 모바일 애플리케이션이 발전함에 따라 가볍고 효율적인 JSON이 현대 소프트웨어 개발의 주류가 되었다.
- **지금은 웹 환경에서 데이터를 교환할 때 JSON이 사실상 표준 기술이다**.

#### Protobuf, Avro

- JSON은 거의 모든 플랫폼에서 호환이 가능하고, 사람이 읽고 쓰기 쉬운 텍스트 기반 포맷이라 디버깅과 개발이 매우 편리하다.
- 하지만 매우 작은 데이터 용량과 극단적으로 빠른 처리 속도가 필요한 환경이라면 **Protobuf**나 **Avro** 같은 기술이 훌륭한 대안이 될 수 있다.
- 이런 기술들은 텍스트가 아닌 순수 **`byte` 기반**으로 동작하기 때문에, 범용적인 호환성은 떨어지지만 용량과 성능 면에서 최적화되어 있어 처리 속도가 매우 빠르다.
- 다만 `byte` 기반이기 때문에 JSON처럼 사람이 문서를 직접 열어보고 읽기는 어렵다는 단점이 있다.

#### 데이터베이스

- 어떤 형식이든 데이터를 파일에 직접 저장하는 방식은 다음과 같은 **큰 한계**가 있다.
  - **데이터 무결성 보장의 어려움**: 여러 사용자가 동시에 파일을 수정하거나 접근하려고 할 때, 데이터의 충돌이나 손상 가능성이 높아져 일관성을 유지하기 어렵다.
  - **비효율적인 검색과 관리**: 특정 형식 없이 저장될 수 있어 필요한 데이터를 빠르게 찾는 데 많은 시간이 소요되며, 데이터 양이 방대해질수록 검색 속도가 급격히 저하된다.
  - **보안 취약성**: 파일 기반 시스템에서는 민감한 데이터를 안전하게 보호하기 위한 접근 제어와 암호화 등이 충분히 구현되지 않아 데이터 유출이나 무단 접근의 위험이 커질 수 있다.
  - **백업 및 복구의 한계**: 대규모 데이터의 효율적인 백업과 복구가 어렵다.
- 이러한 문제점들을 하나하나 해결하면서 발전한 서버 프로그램이 바로 **데이터베이스**다.
- 대부분의 현대 애플리케이션에서는 데이터베이스를 사용하여 파일 시스템의 한계를 극복하고, 대량의 데이터를 효율적으로 저장, 관리, 검색한다.
- 예를 들어 특정 ID를 가진 회원을 찾을 때, 파일에 직접 저장하는 경우 파일의 모든 내용을 찾아봐야 하지만, 데이터베이스는 **인덱스**만 간단하게 지정하면 매우 빠른 속도로 회원을 찾을 수 있다.
- 이런 이유로 실무에서는 대부분의 데이터를 파일에 저장하지 않고 데이터베이스에 저장한다.
- 참고로 이미지, 영상처럼 큰 데이터는 **파일로 보관**하고, 고객 정보, 주문 내역, 결제 내역 등 그 외 거의 모든 핵심 데이터는 **데이터베이스에 보관**한다고 생각하면 된다.
