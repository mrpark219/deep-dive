...
- 대상의 종류나 유형을 나타내는 **서술적 객체**로, 엔티티나 값 객체의 특정 속성을 분류하고 구분하는 용도로 사용한다.
- 예를 들어 전화번호를 값 객체로 모델링했다면, 해당 번호가 집 전화인지, 휴대전화인지, 혹은 회사 전화번호인지 구분하는 식별 기준이 필요하다.
- 과거에는 이를 단순한 숫자나 알파벳 등의 **코드 값**으로 처리했으나, 이는 별도의 매핑 표가 필요하고 코드만으로는 의미를 짐작하기 어려워 가독성과 유지보수성을 크게 떨어뜨린다.
- 따라서 시스템의 맥락에 맞게 누구나 직관적으로 이해할 수 있는 **유비쿼터스 언어를 반영하여 정의하는 것이 바람직하다**.
- 자바(Java) 언어 환경에서는 이를 **열거형(Enum)** 을 활용해 깔끔하게 구현할 수 있다.

#### 애그리거트(Aggregate)

- 엔티티와 값 객체를 활용해 도메인을 모델링하면 자연스럽게 객체 간의 유기적인 계층 구조가 형성된다.
- 이처럼 데이터 변경의 단위로 묶인 **연관된 엔티티와 값 객체들의 논리적인 군집**을 애그리거트라고 한다.
- 애그리거트는 보통 1~2개의 엔티티, 값 객체, 표준 타입 등으로 결합되며, 내부 구성요소들은 강력한 비즈니스 의존관계를 맺고 함께 제어된다. 비즈니스 정합성을 엄격하게 유지해야 하므로 **애그리거트 단위가 데이터 변경 트랜잭션의 기본 단위**가 된다.
- 애그리거트 내의 엔티티 중 가장 상위에 있는 핵심 엔티티를 **애그리거트 루트(Aggregate Root)** 로 지정하며, 외부에서는 오직 이 루트를 통해서만 애그리거트 내부의 객체를 수정하거나 변경할 수 있다.
- 보통 하나의 바운디드 컨텍스트당 하나의 애그리거트가 식별되지만, 비즈니스 영역에 따라 하나의 컨텍스트 안에 여러 개의 애그리거트가 공존하기도 한다.
- 이때 서로 다른 애그리거트 간에 참조가 필요하다면 객체를 직접 참조하지 않고, **애그리거트 루트의 식별자(ID)를 통해 간접 참조**해야 한다.
- 객체를 직접 참조할 경우 애그리거트 단위의 독립적인 트랜잭션 처리가 불가능해지며, 결합도가 높아져 의존 관계가 겉잡을 수 없이 복잡해진다.
- 일반적으로 바운디드 컨텍스트를 기준으로 마이크로서비스를 식별하지만, 개별 애그리거트 역시 독립적인 마이크로서비스의 유력한 후보가 된다. 만약 같은 컨텍스트 내의 여러 애그리거트가 서로의 클래스를 직접 참조하고 있다면 향후 서비스를 분리하는 작업이 매우 어려워진다.
- 따라서 애그리거트 간의 참조는 반드시 **식별자를 활용한 간접 참조 방식을 준수**해야 한다.
- 각 애그리거트는 단일 트랜잭션을 통해 자신 내부의 일관성을 유지하며, 만약 다른 애그리거트와의 정합성이 추가로 요구된다면 **도메인 이벤트를 활용한 결과적 일관성(Eventual Consistency)** 메커니즘을 적용하여 차례로 데이터를 갱신한다.

#### 도메인 서비스(Domain Service)

- 특정 비즈니스 로직이 한 엔티티나 값 객체의 책임으로 귀속시키기 모호하거나, **여러 애그리거트가 협업하여 처리해야 할 때 독립적인 단독 객체**를 정의하는데 이를 도메인 서비스라고 한다.
- 도메인 서비스는 고유의 상태 정보를 관리하지 않고, 오직 **비즈니스 행위(메서드)만 소유**하는 무상태(Stateless) 객체다.
- 따라서 도메인 로직을 수행할 때 스스로 상태를 변경하지 않으며, 인자로 전달받은 **엔티티나 값 객체의 행위를 호출하여 작업을 처리하고 상태 변경 결과 역시 해당 객체들에게 위임**한다.

#### 도메인 이벤트(Domain Event)

- **도메인 주도 설계(DDD) 및 이벤트 스토밍에서 식별한 도메인 이벤트를 실제로 구현한 객체**다. 서비스나 애그리거트 간의 정합성을 맞추기 위해, 상태가 변경된 핵심 애그리거트의 주요 데이터를 담아 전달하도록 모델링한다.
- 이벤트 발행 처리는 비즈니스를 수행하는 **원본 트랜잭션과 반드시 하나의 원자적 단위(Atomicity)로 묶여서 실행**되어야 한다.
- 예를 들어 주문 서비스에서 주문 트랜잭션을 통해 주문 엔티티가 생성 및 저장됨과 동시에 '주문 완료됨' 이벤트를 발행한다. 이 이벤트 객체는 주문 식별자, 결제 금액 등 후속 처리에 필요한 주요 주문 정보를 포함한다.
- 발행된 이벤트는 메시지 브로커를 통해 타 서비스로 전송되며, 이를 수신한 배송 서비스가 독립적으로 배송 프로세스를 시작함으로써 **시스템 간 결합도를 낮추고 결과적 일관성을 달성**한다.
