...
- **ArchUnit**은 자바 코드를 위한 **아키텍처 단위 테스트(Unit Test) 도구**이다.
- 테스트 코드에 **아키텍처 규칙을 선언**하면, ArchUnit이 실제 코드의 의존 관계가 **선언된 규칙을 준수하는지 자동으로 검증**한다.
- ArchUnit을 통해 다음과 같은 다양한 규칙들을 검증할 수 있다.
  - 계층형 아키텍처의 의존 관계 규칙 (예: `Presentation` 계층은 `Domain` 계층에 의존할 수 없다.)
  - 패키지와 클래스 간의 순환 의존성
  - 클래스나 메서드 이름의 형식 (네이밍 컨벤션)
  - 특정 애너테이션의 적용 패턴
  - 의존 관계와 사용 관계의 구분

## 19. 학습 테스트 (Learning Test)

- **학습 테스트(Learning Test)** 는 테스트 코드를 작성하는 과정을 통해, **라이브러리나 레거시 시스템과 같은 외부 코드의 사용 방법을 익히는 기법**이다.
- 학습 테스트는 다음과 같은 장점을 가진다.
  - **코딩 가이드 역할**: 새로운 기술의 사용 방법에 대한 살아있는 문서이자 코딩 가이드가 된다.
  - **변경 감지**: 외부 코드의 버전이 바뀌었을 때, 기존 사용 방식에 변경이 필요한지 학습 테스트를 실행하는 것만으로 쉽게 확인할 수 있다.
  - **높은 ROI**: 학습 테스트에 투자하는 노력보다, 이를 통해 얻는 명확성과 안정성의 성과가 훨씬 더 크다.
