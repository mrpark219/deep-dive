...
- DB가 병목으로 확인되면, 부하가 **읽기 위주인지 쓰기 위주인지에 따라 해결 방향이 달라진다**.
  - **읽기는 복제로 나눌 수 있으나, 쓰기는 복제로 나눌 수 없다**.

```mermaid
flowchart TB
    subgraph R["읽기 부하 → 복제로 분산"]
        direction TB
        R1[읽기 요청] --> R2[캐시<br/>자주 읽는 값 흡수]
        R2 -->|miss| R3[Read Replica<br/>복제본에서 읽기]
        R4[Primary DB<br/>쓰기 담당] -. 복제 .-> R3
    end
    subgraph W["쓰기 부하 → 쪼개어 분산"]
        direction TB
        W1[쓰기 요청] --> W2[샤드 라우터<br/>샤드 키로 분배]
        W2 --> W3[Shard 1]
        W2 --> W4[Shard 2]
    end
```

### 8.1. 읽기 부하: 캐시와 Read Replica

- 먼저 **캐시**가 자주 읽는 값을 받아 내어 DB 접근 자체를 줄인다.
- 캐시에 값이 없으면(miss), **Read Replica**로 읽기 요청을 보낸다.
  - Read Replica는 Primary를 복제한 읽기 전용 사본이다.
  - Primary는 쓰기만 담당하므로 읽기 트래픽에서 자유로워진다.
- 다만 Primary에서 Replica로 데이터가 복제되기까지 시간 차인 **복제 지연(Replication Lag)** 이 존재한다.
  - 방금 쓴 값을 곧바로 읽어야 하는 경우에는 Primary에서 직접 읽어야 한다.

### 8.2. 쓰기 부하: 샤딩

- Read Replica는 쓰기 분산에 도움이 되지 않는다.
  - 모든 복제본에 같은 쓰기를 똑같이 반영해야 하므로 쓰기가 늘수록 오히려 부담만 커진다.
- 그래서 데이터를 **샤드 키**(예: 사용자 ID)를 기준으로 여러 조각으로 나누고, 각 조각(샤드)이 자기 몫의 쓰기만 담당하게 한다.
  - 이렇게 하면 쓰기 처리량이 샤드 수에 비례해 늘어난다.
- 다만 샤드 키를 잘못 고르면 특정 샤드에만 부하가 몰리는 **핫스팟(Hotspot)** 이 생긴다.
  - 또한 여러 샤드에 걸친 조인이나 트랜잭션이 어려워지므로, 데이터를 나누는 기준을 처음부터 신중히 정해야 한다.

### 8.3. 전략 비교

| 부하 유형     | 전략                | 핵심 원리                       | 주의점                         |
| :------------ | :------------------ | :------------------------------ | :----------------------------- |
| **읽기 많음** | 캐시 + Read Replica | 복제본을 늘려 읽기를 분산한다   | 복제 지연, 캐시 정합성         |
| **쓰기 많음** | 샤딩                | 데이터를 쪼개어 쓰기를 분산한다 | 샤드 키 선정, 크로스 샤드 연산 |
