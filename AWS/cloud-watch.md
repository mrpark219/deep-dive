...
- AWS 계정의 **거버넌스**, **규정 준수**, **운영 감사**, **위험 감사**를 지원하는 서비스이다.
- AWS 인프라에서 계정 활동과 관련된 작업을 기록하고 지속적으로 **모니터링**하며 보관한다.
- AWS Management Console, SDK, 명령줄 도구 등을 통해 수행된 작업을 비롯하여 **이벤트 기록**을 제공한다.
- AWS의 **보안** 및 **감사**를 위한 서비스(감시 **CCTV**)이다.
- AWS의 여러 서비스에 대한 **활동 이벤트 로그** 등을 제공한다.
  - 이벤트 예시: **EC2 종료**, **S3 버킷 생성**, **IAM 사용자 삭제** 등
  - 이벤트의 **시간**, **결과**, **에러**, **사용 인증 정보** 등을 기록한다.
  - AWS CLI, 콘솔, API 호출 등 **모든 이벤트**가 대상이다.
- 기본적으로 **90일**의 이벤트 로그를 무료로 저장하며, 이 이상의 기록은 **Trail** 생성이 필요하다.
- **실시간 기록**이 아니며 약 **5분**의 지연 시간이 발생한다.

### 8.1. AWS CloudTrail Trail

- **S3** 기반의 이벤트 로그 **수집 단위**이다. 즉 **S3 버킷** 생성 후 지정이 필요하다.
- **리전 기반**이며, **단일 리전** 혹은 **다중 리전** 선택이 가능하다.
  - **다중 리전**일 경우 모든 리전의 로그를 한 곳으로 모아 관리 가능하며, 신규 리전 추가 시 **자동**으로 로깅된다.
  - **글로벌 서비스**(IAM, STS, CloudFront)의 경우 **us-east-1** 리전에서 로깅된다.
    - 다중 리전 로깅 혹은 us-east-1에 Trail 생성이 필요하다.
- 별도로 **EventBridge**, **CloudWatch** 등 AWS의 다양한 서비스로 이벤트 전달 설정이 가능하다.
  - **Metric Filter** 및 **이벤트 트리거**가 가능하다.
  - **5분 이상**의 지연 시간 가능성을 염두에 두어야 한다.

### 8.2. AWS CloudTrail Event

- AWS 계정의 **활동 내용**을 기록한다.
  - 몇몇 **Data API**의 경우 수동으로 활성화가 필요하다(S3, Lambda, DynamoDB 등).
  - **JSON 형식**으로 데이터를 저장하며 **Athena** 등으로 분석 가능하다.
- 총 **세 가지 종류**가 있다.

#### Management Event

- AWS 계정의 **관리**를 위한 이벤트이다.
  - **IAM 역할 부여**, **VPC 생성/삭제**, **서브넷 생성/삭제**, **Trail 생성**, **로그인 이벤트** 등
- **기본 로깅 대상**이다.

#### Data Event

- 리소스의 **동작**과 관련된 데이터이다.
  - **S3 Object** 동작(Get, Delete, Put), **Lambda** 함수 호출, **SNS** 호출 등
- 별도로 **로깅 활성화**가 필요하며 **추가 비용**이 발생한다.

#### Insight Event

- CloudTrail에서 별도로 생성하는 **비정상적**인 상황에 대한 이벤트이다.
  - 예: 평소와 다른 S3 버킷 100개 삭제, Deny 된 EC2 인스턴스 프로비전 요청이 분당 수십 개 발생 등
- 별도로 **로깅 활성화**가 필요하며 **추가 비용**이 발생한다.

### 8.3. CloudWatch와 CloudTrail

#### CloudWatch

- AWS를 **모니터링** 하기 위한 서비스이다(**퍼포먼스 체크**).
- AWS 서비스뿐만 아니라 애플리케이션의 **로그** 및 **동작 로그**를 취합한다.
- 애플리케이션이 **어떻게** 동작하였는가, 무슨 **버그**가 있었는가, **메모리**는 얼마나 소모되었는가 등을 확인한다.
- **대시보드** 및 **알람** 등 모니터링을 위한 서비스를 제공한다.

#### CloudTrail

- AWS를 **감사(Audit)** 하기 위한 서비스이다(**감시 CCTV**).
- AWS의 모든 서비스가 사용될 때마다 **사용 로그**를 저장한다.
- AWS가 **언제**, **어디서**, **누구**에 의해 사용되었는가 등을 확인한다.
- 단순하게 AWS **사용 로그**만 저장한다.
