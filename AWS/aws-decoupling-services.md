...
- Visibility Timeout이 만료되면 메시지는 다시 **처리 가능한 상태(맨 뒤로 복귀)**로 돌아간다.
...
![https://docs.aws.amazon.com/ko_kr/streams/latest/dev/images/architecture.png](./images/aws-decoupling-services/2025-12-27-21-07-55.png)

- **실시간 스트리밍 데이터를 손쉽게 수집, 처리 및 분석**하여 통찰력을 확보하고 새로운 정보에 신속하게 대응할 수 있는 서비스이다.
- 모든 규모의 스트리밍 데이터를 **비용 효율적**으로 처리할 수 있는 핵심 기능과 더불어, 애플리케이션 요구 사항에 가장 적합한 도구를 선택할 수 있는 유연성을 제공한다.

### 5.1. Amazon Kinesis 서비스

- **Kinesis Data Stream**: 스트리밍 데이터를 **수집**하는 서비스이다.
- **Amazon Data Firehose**: 데이터를 수집하고 변환하여 **공급자**(S3, RedShift, AWS OpenSearch, Datadog, HTTP 등)에게 **전송**하는 서비스이다.
- **Kinesis Data Analytics Studio**: 실시간 스트리밍 데이터를 **분석**하는 서비스이다.
- **Kinesis Video Streams**: 재생, 분석 및 기계 학습을 위해 **미디어 스트림**을 캡처, 처리 및 저장한다.

### 5.2. Kinesis Data Stream

- 스트리밍 데이터를 수집하고 처리 주체에 **전달**하는 서비스이다.
  - 요청이 아닌 **데이터**의 처리를 담당한다.
  - 단발성 데이터가 아니라 많은 분량으로 지속적으로 전달되는 데이터(**로그**, **이용 데이터**, **센서 데이터**)를 처리한다.
  - 작은 데이터부터 거의 **무한대**의 용량을 처리 가능하며, **고가용성**이 내부적으로 확보되어 있다.

#### 주요 구성 요소

- **Data Stream**: **샤드(Shard)** 로 구성된 데이터 처리를 위한 스트림이다.
- **데이터 레코드(Data Record)**: Kinesis를 활용해 보내려 하는 **데이터**이다.
  - **Sequence Number**: 샤드 내의 **파티션 키** 단위로 부여되는 고유 번호이다.
- **Producer**: 데이터를 생산해서 Kinesis에 **전달**하는 주체이다.
- **Consumer**: 데이터를 **소비**하는 주체로, 데이터를 직접 처리하거나 다른 주체로 전달한다.
- **액세스 정책**: Kinesis의 **접근**을 제어한다.

#### Data Stream

- 데이터 처리를 위한 **스트림**이다.
- **24시간**에서 최대 **1년** 단위로 데이터를 저장한다(SQS는 14일).
  - SQS와 달리 원한다면 이 기간 내에서 데이터를 **다시 조회(Replay)** 할 수 있다.
- **샤드** 단위로 데이터를 처리한다.
- **Capacity Mode**
  - **On-Demand**: 샤드를 수요에 따라 증설하고 사용한 데이터 **용량**만큼 비용을 지불한다.
  - **Provisioned**: **샤드 숫자**를 지정하고(변경 가능) 총 숫자만큼의 샤드에 대해서만 비용을 지불한다.

#### Shard

- Kinesis의 **Data Stream 처리 단위**로 일종의 내부 파이프라인이다.
- 정해진 **용량**의 데이터를 처리한다.
  - **읽기**: 초당 **5 Transactions**, **2MB** 처리 가능하다.
  - **쓰기**: 초당 **1000 데이터 레코드**, **1MB** 처리 가능하다.
  - 즉 Data Stream의 처리 가능 용량은 보유한 **샤드의 합**으로 계산한다.
- 데이터 레코드의 **Partition Key**의 **MD5 Hash** 값을 기반으로 샤드를 결정한다.
- **샤드 숫자 조절**이 가능하다.
  - **Reshard**: 2개로 나누는 **Split**, 2개를 합치는 **Merge**로만 가능하다.
  - **Update Shard**: **Background**에서 샤드 숫자를 지정한 숫자로 조절(병합/분할)한다.
  - **OPEN**, **CLOSED**, **EXPIRED** 상태로 관리된다.

#### Producer

- **3가지 방법**으로 Kinesis로 데이터를 전송한다.
  - **API**: **PutRecord/PutRecords**로 데이터 레코드를 전달한다.
  - **Amazon Kinesis Producer Library(KPL)**: Kinesis에 데이터 레코드를 전달하기 위한 **라이브러리**이다.
    - Stream에 데이터를 전달하기 위한 다양한 기능(**Retry**, **데이터 취합** 및 **Batch 전달**, **CloudWatch 메트릭**을 통한 모니터링)을 제공한다.
  - **Amazon Kinesis Agent**: **Java** 기반 애플리케이션이다.

#### Consumer

- **KCL(Amazon Kinesis Client Library)**: Kinesis의 Data Record 읽기를 쉽게 관리하기 위해서 만들어진 **라이브러리**이다.
  - **Java**, **Python**, **Ruby**, **Node.js**, **.NET**을 지원한다.
  - 스트림 연결, 데이터 **스트림의 변경**, 데이터 스트리밍의 **로드 밸런싱**, **리샤드** 등의 다양한 상황 및 로직 처리 기능을 미리 구현했다.
- 직접 데이터를 처리하기도 하고 상황에 따라서는 다른 주체(**S3**, **OpenSearch** 등)에 전달한다.

#### 기타

- **KMS**를 활용한 **서버 사이드 암호화**를 지원한다.
- **IAM 리소스 기반 정책**으로 권한 부여가 가능하다.
