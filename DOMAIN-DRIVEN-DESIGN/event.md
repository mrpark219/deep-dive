...
- 이벤트를 시스템에 적용할 때는, 단순히 발행하고 처리하는 것 외에 다음과 같은 점들을 추가로 고려해야 한다.
  - **이벤트 소스(Source) 정보**: 특정 주체가 발생시킨 이벤트만 따로 추적해야 한다면, 이벤트에 발생 주체 정보를 포함해야 한다.
  - **전송 실패와 재시도 정책**: 이벤트 전송에 계속 실패할 경우, 해당 이벤트 때문에 다른 이벤트까지 막히는 것을 방지하기 위해 **최대 재전송 횟수 제한**과 같은 정책이 필요하다.
  - **이벤트 손실**: 로컬 핸들러를 비동기로 처리할 경우 이벤트 처리 실패 시 유실될 수 있다. 반면 **이벤트 저장소를 사용하면 트랜잭션 내에 이벤트 저장을 보장**하여 손실을 막을 수 있다.
  - **이벤트 순서**: 이벤트 발생 순서대로 처리하는 것이 중요하다면, **순서를 보장해주는 이벤트 저장소나 메시지 큐를 사용**해야 한다. (일부 메시징 시스템은 순서를 보장하지 않는다.)
  - **이벤트 재처리(멱등성)**: 동일한 이벤트가 여러 번 전달되더라도 문제가 발생하지 않도록, **핸들러를 멱등(Idempotent)하게 구현**하거나, 이미 처리한 이벤트인지 순번을 확인하는 로직이 필요하다.

### 6.1. 이벤트 처리와 DB 트랜잭션 고려

- DB 트랜잭션이 성공적으로 커밋된 직후에 이벤트 핸들러가 실패하는 경우가 있다. 예를 들어, '주문 취소'는 DB에 반영되었지만, 뒤이어 실행된 '환불' 이벤트 핸들러가 외부 API 연동 실패로 예외를 발생시키면 **데이터 불일치 상태**에 빠지게 된다.
- 이러한 복잡성을 줄이는 가장 좋은 방법은, **DB 트랜잭션이 성공적으로 커밋될 때만 이벤트 핸들러를 실행**하는 것이다.
- 스프링은 이를 위해 **`@TransactionalEventListener`** 애너테이션을 제공한다. 이 애너테이션이 붙은 핸들러는, **현재 트랜잭션이 성공적으로 커밋된 후에만 실행**된다. 트랜잭션이 롤백되면 핸들러는 아예 실행되지 않는다.
- 이벤트 저장소를 사용하는 방식도 동일한 효과를 낸다. **도메인 상태 변경과 이벤트 저장을 하나의 트랜잭션**으로 묶으면, 트랜잭션이 성공할 때만 이벤트가 저장소에 기록되기 때문이다.
- 이처럼 트랜잭션과 이벤트 실행을 연계하면, **'트랜잭션은 실패했는데 이벤트는 실행되는' 경우의 수를 원천적으로 제거**할 수 있다. 그 결과 개발자는 **'이벤트 처리 실패' 하나만 고민**하면 되므로 시스템의 복잡도가 크게 낮아진다.
